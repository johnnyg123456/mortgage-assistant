...
async function generateDraftBody(email, classification, styleCtx) {
  ...
  // Inject up to 5 recent draft corrections as examples
  let correctionExamples = '';
  try {
    const fb = loadFeedback();
    const recent = (fb.draftCorrections ?? []).slice(0, 5);
    if (recent.length) {
      correctionExamples = '\n\nPAST CORRECTIONS FROM JOHN (use these as style examples):\n' +
        recent.map((c, i) =>
          `${i + 1}. Context: ${c.originalContext}\n   John said: "${c.johnNote}"\n   Correct reply: "${c.correctedDraft}"`
        ).join('\n\n');
    }
  } catch { /* non-fatal */ }

  // Skip draft if John marked this sender as no-draft
  try {
    const fb = loadFeedback();
    const fromLower = (email.from ?? '').toLowerCase();
    if ((fb.noDraftSenders ?? []).some(s => fromLower.includes(s.toLowerCase()))) {
      log('no-draft-sender', { from: email.from });
      return null;
    }
  } catch { /* non-fatal */ }
...
async function handle(email, classification, styleCtx) {
  ...
  try {
    ...
    if (!draftBody) {
      log('skipped-empty-body', { messageId: email.messageId });
      return null;
    }
