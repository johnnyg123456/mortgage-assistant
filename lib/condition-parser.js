...
async function maybeDraftBorrowerConditionsEmail({ loanName, loanNumber, addedList }) {
  if (!addedList?.length) return;
  try {
    const { draftForNewConditions } = require('./borrower-conditions-drafter');
    const draftResult = await draftForNewConditions({ loanName, loanNumber, addedList });
