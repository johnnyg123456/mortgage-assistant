...
function isLoForward(from) {
  ...
  if (johnEmail && addr === johnEmail) return false;
  if (christyEmail && addr === christyEmail) return false;
  if (isBrokerEmail(from)) return true;
  return ALLOWED_EXTERNAL_LOS.includes(addr);
}
...
function isEligibleDocSender(from, subject, body) {
  ...
  // John/Christy self-sent mail is excluded inside isLoForward.
