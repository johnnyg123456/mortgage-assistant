...
function buildDigestEmail(items) {
  ...
  const urgentDirect = urgent.filter(i => !i.isCc);
  const urgentCc     = urgent.filter(i => i.isCc);

  if (urgentDirect.length) {
    lines.push('━━━ URGENT ━━━');
    urgentDirect.forEach((item, i) => {
  ...
  if (urgentCc.length) {
    lines.push('━━━ URGENT (CC\'D — FYI ONLY) ━━━');
    urgentCc.forEach((item, i) => {
      lines.push(`${i + 1}. ${item.summary}`);
      lines.push(`   From: ${item.from}`);
      lines.push(`   Subject: ${item.subject}`);
      lines.push(`   You were CC'd — no reply needed, but worth a look.`);
      lines.push('');
    });
  }
